    ...
    <div class="container mt-5 py-5">
      <div class="row">
        ...
        if (isset($_GET['title']) && isset($_GET['year'])) {
          ...
          $seriesYear = $_GET['year'];

          // Query movie_details table to fetch details for the selected movie
          $detailsSql = "SELECT * FROM series_details WHERE title = ? AND year = ?";
          $detailsStmt = $conn->prepare($detailsSql);
          $detailsStmt->bind_param("si", $seriesTitle, $seriesYear);
          $detailsStmt->execute();
          ...
          if ($detailsResult->num_rows > 0) {
            ...
            echo '<p><strong>Season(s):</strong> ' . $detailsRow['season'] . '</p>';
            echo '<p><strong>Episodes:</strong> ' . $detailsRow['episodes'] . '</p>';
